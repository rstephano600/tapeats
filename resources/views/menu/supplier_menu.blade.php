...
<body class="bg-light">
    ...
    <div class="d-flex align-items-center mb-5 p-4 rounded-3 shadow-sm bg-white">
        @if($supplier->logo)
        <img src="{{ asset('storage/' . $supplier->logo) }}" alt="{{ $supplier->business_name }}" class="rounded-circle me-4" style="width: 80px; height: 80px; object-fit: cover;">
        @else
        <div class="rounded-circle bg-gradient-custom d-flex align-items-center justify-content-center me-4" style="width: 80px; height: 80px;">
            <span class="text-white h3 fw-bold">{{ substr($supplier->business_name, 0, 2) }}</span>
        </div>
        @endif
        
        <div>
            <h1 class="h3 fw-bold mb-1">{{ $supplier->business_name }}</h1>
            <p class="text-muted mb-1">{{ $supplier->description ?? 'No description available.' }}</p>
            <span class="small text-secondary">📍 {{ $supplier->region ?? 'Location' }}</span>
        </div>
    </div>

    {{-- Foods Grid --}}
    <h2 class="h5 fw-bold text-dark mb-4">Menu Items</h2>
    
    @if($foods->isEmpty())
        <div class="alert alert-info" role="alert">
            This supplier currently has no food items listed.
        </div>
    @else
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4" id="foodsGrid">
            @foreach($foods as $food)
                <div class="col">
                    <div class="card h-100 shadow-sm border-0 rounded-3 overflow-hidden">
                        {{-- Food Image/Details here (Similar to your main menu view) --}}
                        <div class="card-body p-3">
                            <h5 class="card-title fw-bold mb-1">{{ $food->name }}</h5>
                            <p class="card-text small text-muted">{{ $food->description }}</p>
                            <div class="d-flex justify-content-between align-items-center mt-3">
                                <span class="h6 fw-bolder text-custom-orange">
                                    ${{ number_format($food->price, 2) }}
                                </span>
                                
                                {{-- ADD TO CART BUTTON --}}
                                {{-- Assuming a JavaScript function 'addToCart' is available globally --}}
                                <button onclick="addToCart({{ $food->id }}, '{{ $food->name }}', {{ $food->price }})" 
                                        class="btn btn-sm btn-custom-orange fw-semibold">
                                    + Add to Cart
                                </button>
                            </div>
                        </div>
